...
class CategoryModel extends ListModel
{
	...
	/**
	 * Array of test1 in the category
	 *
	 * @var    \stdClass[]
	 */
	...
	/**
	 * The list of other test1 categories.
	 *
	 * @var    array
	 */
	...
	public function getItems()
	{
		...
		for ($i = 0, $n = count($items); $i < $n; $i++) {
			...
			if ($this->getState('load_tags', true)) {
				...
				$this->tags->getItemTags('com_test1.test1', $item->id);
			}
		}
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$numberOfItemsToDisplay = $mergedParams->get('test1_display_num');

		if ($format === 'feed') {
			$limit = $app->get('feed_limit');
		} else if (isset($numberOfItemsToDisplay)) {
		...
		$search = $app->getUserStateFromRequest('com_test1.category.list.' . $itemid . '.filter-search', 'filter-search', '', 'string');
	...
	public function getCategory()
	{
		if (!is_object($this->_item)) {
			...
			$categories = Categories::getInstance('Test1', $options);
